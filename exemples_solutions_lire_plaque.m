...
function detectLicensePlate(imagePath)
    % Charger l'image
    img = imread(imagePath);
    ...
    % Définir une plage de seuillages binaires
    thresholds = linspace(0.1, 0.9, 5); % Modifier la plage et le nombre de seuillages si nécessaire
    ...
    % Appliquer une boucle sur les différentes valeurs de seuillage
    for i = 1:length(thresholds)
        % Appliquer le seuillage binaire
        binaryImg = imbinarize(imgGray, thresholds(i));

        % Appliquer une opération de fermeture pour connecter les régions
        se = strel('disk', 5);
        closedImg = imclose(binaryImg, se);

        % Imbriquer une boucle pour rechercher des régions de différentes tailles
        subplot(2, 3, i + 1);
        imshow(img);
        hold on;

        % Trouver les régions d'intérêt dans l'image
        stats = regionprops(bwlabel(closedImg), 'BoundingBox', 'Area');

        % Filtrer les régions en fonction de la taille
        minArea = 1000; % Ajuster la taille minimale selon besoin
        maxArea = 50000; % Ajuster la taille maximale selon besoin
        validRegions = stats([stats.Area] > minArea & [stats.Area] < maxArea);

        % Dessiner les rectangles rouges autour des régions d'intérêt
        for j = 1:numel(validRegions)
            rectangle('Position', validRegions(j).BoundingBox, 'EdgeColor', 'r', 'LineWidth', 2);
        end

        title(['Seuil: ' num2str(thresholds(i))]);
        hold off;
    end

    sgtitle('Détection de régions d''intérêt avec différents seuillages');
end
...
function validText = detectLicensePlateWithOCR(imagePath)
    ...
    % Initialiser la variable pour stocker le texte détecté
    detectedText = '';
    ...
    for i = 1:length(thresholds)
        ...
        for j = 1:numel(validRegions)
            ...
            roi = imcrop(imgGray, validRegions(j).BoundingBox);

            % Appliquer la lecture OCR
            ocrResult = ocr(roi);

            % Ajouter le texte à la variable detectedText si non vide
